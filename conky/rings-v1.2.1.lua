...
+ v1.0 -- Original release (28.09.2009)
        ...
        arg=conky_parse("${if_up wlp2s0}wlp2s0${else}eth0${endif}"),
        fg_colour=0xf0651f,
        fg_colour=conky_parse("${if_up wlp2s0}wlp2s0${else}eth0${endif}"),
        conky_parse("${cpu}")
...
settings_table = {
    ...
    {
        name='exec nvidia-settings -q [gpu:0]/GPUUtilization -t | cut -d "=" -f 2 | cut -d "," -f 1',
        arg='',
        max=100,
        bg_colour=0xdddddd,
        bg_alpha=0.6,
...
function iface_watch()
    ...
    iface=conky_parse("${if_existing /proc/net/route eth0}eth0${else}wlp2s0${endif}")
